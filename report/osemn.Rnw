...
<<chunk1>>=
# Chunk1 shows the codes for downloading the dataset from a 
...
<<chunk2>>=
# Chunk2 shows the codes for extracting the relevant rows 
# and columns.
...
<<chunk3>>=
# Chunk3 shows the codes for changing the columns' name and 
# displaying the data.
...
<<chunk8, figure=TRUE>>=
# Chunk8 has the codes for melting the dataset and plotting 
# the line graph.
# Melting the data
library(reshape2)
...
ggplot(moltenData, aes(as.Date(year,"%e %b %Y"),value))+ 
    ...
    teachers in Chicago,IL")+theme_bw() 

# Figure shows a line graph showing the total schools, total 
# students and total teachers from 2001 to 2011.
@
